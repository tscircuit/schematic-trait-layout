...
export const getGridFromCircuit = (circuit: CircuitBuilder): Grid => {
  ...
  for (const chip of circuit.chips) {
    const totalPins = chip.leftPinCount + chip.rightPinCount + chip.topPinCount + chip.bottomPinCount;
    const isHorizontalPassive = chip.leftPinCount === 1 && chip.rightPinCount === 1 && chip.topPinCount === 0 && chip.bottomPinCount === 0;
    const isVerticalPassive = chip.topPinCount === 1 && chip.bottomPinCount === 1 && chip.leftPinCount === 0 && chip.rightPinCount === 0;
    const isTwoPinPassive = totalPins === 2 && (isHorizontalPassive || isVerticalPassive);
    ...
    for (let r = 0; r < height; ++r) { // r is visual row index from bottom (0) to top (height-1)
      ...
      if (isBottomBorder) {
        ...
        const leftPinData = (pinSlotDisplayIndex < chip.leftPinCount)
          ? chip.leftPins[chip.leftPinCount - 1 - pinSlotDisplayIndex]
          : undefined;
        const rightPinData = (pinSlotDisplayIndex < chip.rightPinCount)
      ...
      for (let col = 0; col < rowStr.length; ++col) { // rowStr.length is 5
  ...
  for (const line of circuit.lines) {
    if (line.start.x === line.end.x) { // Vertical line
      ...
      g.addEdge(x, y_start_coord, y_start_coord < y_end_coord ? "down" : "up");
      g.addEdge(x, y_end_coord, y_start_coord < y_end_coord ? "up" : "down");
      ...
      g.addEdge(x_start_coord, y, x_start_coord < x_end_coord ? "right" : "left");
